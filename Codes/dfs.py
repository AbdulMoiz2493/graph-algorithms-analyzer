...
class Graph:
    ...
    @measure_time
    def dfs(self, start_node, trace_file):
        visited = set()
        stack = [start_node]
        traversal = []
        trace = [f"DFS Trace from {start_node}\n"]

        while stack:
            node = stack.pop()
            trace.append(f"Popped from stack: {node}\n")
            if node not in visited:
                ...
                for neighbor in reversed(self.graph[node]):
                    if neighbor not in visited:
                        ...
                        trace.append(f"Pushed to stack: {neighbor}\n")

        with open("dfs_traversal.txt", "w") as f:
            f.write(f"DFS Traversal: {' -> '.join(map(str, traversal))}\n")
        print(f"DFS Traversal: {' -> '.join(map(str, traversal))}")
        ...
        plt.title(f"DFS Traversal from {start_node}")
        plt.savefig(f"dfs_traversal_{start_node}.png")
        plt.close()
    ...
    def analyze_runtime(self):
        ...
        algorithms = [
            ("DFS", lambda g: g.dfs(min(g.nodes), "dfs_temp_trace.txt"))
        ]
        ...
        edge_factor = 2
        times = []
        print("Analyzing DFS for sparse directed graph...")
        for n in node_counts:
            ...
            result, time_taken = self.dfs(0, "dfs_temp_trace.txt")
            times.append(time_taken)

        plt.figure(figsize=(12, 8))
        plt.plot(node_counts, times, color='blue')
        plt.title("Graph of Execution time of DFS")
        plt.xlabel("Input Size (Number of Nodes)")
        plt.ylabel("Running Time (seconds)")
        plt.grid(True)
        plt.xlim(10, 100)
        plt.ylim(0, max(times) * 1.2 if times else 1)
        plt.savefig("runtime_dfs.png")
        plt.close()
        print("Plot saved: runtime_dfs.png")
...
def main():
    ...
    source = int(input("Enter source node for DFS: "))
    if source not in g.nodes:
        print(f"Node {source} not in graph. Exiting.")
        return

    traversal, time_taken = g.dfs(source, "dfs_trace.txt")
    print(f"DFS execution time: {time_taken:.6f} seconds")
    with open("dfs_execution_time.txt", "w") as f:
        f.write(f"DFS: {time_taken:.6f} seconds\n")

    print("Running runtime analysis...")
    g.analyze_runtime()
    print("Runtime analysis complete. Plots saved as all_nodes_visualization.png, visualization_100_200_nodes.png, runtime_comparison_100_200_nodes.png, and runtime_dfs.png")
